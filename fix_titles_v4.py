...
"""
最终版title修复 v4
基于实际页面结构分析，修复v3中的误抓问题：
1. 从面包屑导航a标签提取时，只提取当前导师所在分类（最后一个a标签）
2. 排除导航菜单中的"院长""主任"等误抓
3. 对特定导师手动修正
"""
import json
import os
...
MANUAL_FIXES = {
    "柳晶晶": {
        "title": ["教授", "副教授", "硕士生导师", "副院长"],
        "supervisor_type": "硕士生导师"
    },
    "梁忠伟": {
        "title": ["教授", "博士生导师", "硕士生导师", "主任"],
        "supervisor_type": "博士生导师"
    },
    "邹涛": {
        "title": ["教授", "博士生导师", "院长"],
        "supervisor_type": "博士生导师"
...
def extract_titles(html, teacher_name):
    """
    从页面中提取职称，基于实际页面结构
    """
    ...
    # 策略4: 从面包屑导航的最后一个a标签提取（当前分类）
    # 面包屑通常是：首页 > 师资队伍 > 教师名录 > 机电工程系 > 教授（正高）
    all_links = [a.get_text().strip() for a in soup.find_all('a')]
    # 只检查可能包含职称的链接（短文本）
    for link in all_links:
        if len(link) < 20:  # 短链接才可能是分类
            for t in ALL_TITLES:
...
def main():
    ...
    for i, teacher in enumerate(teachers):
        ...
        if name in MANUAL_FIXES:
            new_titles = MANUAL_FIXES[name]['title']
            teacher['title'] = new_titles
            teacher['supervisor_type'] = MANUAL_FIXES[name]['supervisor_type']
            if old_titles != new_titles:
                fixed_count += 1
                print(f"  手动修正: {new_titles}")
                print(f"  [已更新]")
            continue
        ...
        new_titles = extract_titles(html, name)
