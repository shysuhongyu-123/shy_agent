...
"""
最终版title修复：
1. 保留所有行政职务（院长、副院长、主任、副主任等）
2. 但必须确保数据准确，只从页面标题/顶部信息区提取
3. 不从个人简介正文误抓
4. 排除"博导""硕导"等冗余缩写
"""
import json
...
def extract_titles_safe(html, teacher_name):
    """
    安全提取职称，避免从个人简介误抓
    策略：
    1. 只从页面标题 <title> 提取
    2. 只从 <h1>/<h2>/<h3> 标题提取
    3. 只从页面顶部前300字符提取（通常是个人信息区）
    4. 不从个人简介正文提取
    """
    ...
    for tag in ['h1', 'h2', 'h3', 'h4']:
        for el in soup.find_all(tag):
            el_text = el.get_text()
            for t in ALL_TITLES:
                if t in el_text and t not in found_titles:
                    found_titles.append(t)
    
    # 策略3: 从页面顶部前300字符提取（个人信息区，非简介正文）
    text = soup.get_text()
    page_top = text[:300]
    for t in ALL_TITLES:
        if t in page_top and t not in found_titles:
            found_titles.append(t)
...
def main():
    ...
    for i, teacher in enumerate(teachers):
        ...
        new_titles = extract_titles_safe(html, name)
